...
int error_exit(char **ar)
{
    int i = 0;
    int j = 0;
    if ((ar[1] && my_str_isnum(ar[1]) != 1) || (ar[1] && ar[2])) {
        for (; ar[1][i] && issign(ar[1][i]); i++);
        for (j = i; ar[1][j] && isnum(ar[1][j]); j++);
        if (j != 0 && isnum(ar[1][j - 1]) && my_str_isnum(ar[1]) == 0)
