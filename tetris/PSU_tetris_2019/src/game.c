...
int gameloop(char *input_c, game *info, char **game_array)
{
    long speed = 1000000;
    long tmp_time = 0;
    for (; create_new_tetri(info, game_array) == RUNNING; print_extras(info)) {
        ...
        info->gametime = clock() - info->starttime - info->pause_time;
        if (info->gametime > tmp_time && info->cur && info->pause == FALSE) {
            tmp_time = info->gametime + speed - info->level * 10000;
            change_position(game_array, info, 0, 1);
        }
        if (my_strlen(input_c) > 0) if(key_event(game_array, input_c, info)
        == 84)
                return (SUCCESS);
