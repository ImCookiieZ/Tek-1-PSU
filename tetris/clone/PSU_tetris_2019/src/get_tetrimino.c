...
void parse_tetrimino(game *info)
{
    ...
    for (int i = 0; i < info->count_tetri; i += 1, close(fd), free(path)) {
        ...
        if ((fd = open(path, O_RDONLY)) == -1) my_error_exit("OPEN");
        if (stat(path, &st) == -1) my_error_exit("stat failed\n");
        if (st.st_size > 199) my_error_exit("too big file\n");
        my_memset(buffer, 199, 0);
        if ((read(fd, buffer, st.st_size)) == -1) my_error_exit(("READ"));
        buffer[st.st_size] = '\0';
